...
double benchmark_blake_256(uint8_t *input, uint64_t input_len, int number_of_tests){
    blake_BitSequence output[64];
...
int main(){
    ...
    for(int i=8; i< 9; i++){
        ...
        blake_time = benchmark_blake_256(input, input_len[i], number_of_tests);
        printf("blake_256  time: "GRN"%f"reset"\n", blake_time);
