#include <string.h>
#include <stdio.h>
#include "include/blake_opt32.h"


static  blake_HashReturn compress32( blake_hashState * state, const blake_BitSequence * datablock ) {
...
static blake_HashReturn compress64( blake_hashState * state, const blake_BitSequence * datablock ) {

#define ROT64(x,n) (((x)<<(64-n))|( (x)>>(n)))
#define ADD64(x,y)   ((blake_u64)((x) + (y)))
#define XOR64(x,y)    ((blake_u64)((x) ^ (y)))
...
blake_HashReturn blake_Init( blake_hashState * state, int hashbitlen ) {
...
blake_HashReturn blake_AddSalt( blake_hashState * state, const blake_BitSequence * salt ) {
  ...
  /* blake_FAIL if Init() was not called before */
  if (state->init != 1) 
    return blake_FAIL;
...
static blake_HashReturn Update32(blake_hashState * state, const blake_BitSequence * data, blake_DataLength databitlen ) {
...
static blake_HashReturn Update64(blake_hashState * state, const blake_BitSequence * data, blake_DataLength databitlen ) {
...
blake_HashReturn blake_Update(blake_hashState * state, const blake_BitSequence * data, blake_DataLength databitlen ) {
...
static blake_HashReturn Final32( blake_hashState * state, blake_BitSequence * hashval ) {


  unsigned char msglen[8];
  blake_BitSequence zz=0x00,zo=0x01,oz=0x80,oo=0x81;
  ...
  */
  ...
  if ( state->datalen % 8 == 0) {
	...
	state->data32[state->datalen/8] ^= 0x01;
      ...
      state->datalen = (state->datalen&(blake_DataLength)0xfffffffffffffff8ULL)+8;
    }
    else { 
      if (state->datalen < 440) {
	/* enough space to fill the block */
	state->t32[0] -= 440 - state->datalen;
	state->datalen = (state->datalen&(blake_DataLength)0xfffffffffffffff8ULL)+8;
	blake_Update( state, padding+1, 440 - state->datalen );
      }
      else { 
...
static blake_HashReturn Final64( blake_hashState * state, blake_BitSequence * hashval ) {
  ...
  if ( state->datalen % 8 == 0) {
	...
	blake_Update( state, &zo, 8 );
      state->t64[0] -= 8;
    }
    state->t64[0] -= 128;
    blake_Update( state, msglen, 128 );    
  }
  else {  
    /* message bitlength NOT multiple of 8 */
  ...
  blake_u64TO8_BE( hashval + 0, state->h64[0]);
  blake_u64TO8_BE( hashval + 8, state->h64[1]);
  blake_u64TO8_BE( hashval +16, state->h64[2]);
  blake_u64TO8_BE( hashval +24, state->h64[3]);
  blake_u64TO8_BE( hashval +32, state->h64[4]);
  blake_u64TO8_BE( hashval +40, state->h64[5]);

  if ( state->hashbitlen == 512 ) {
    blake_u64TO8_BE( hashval +48, state->h64[6]);
    blake_u64TO8_BE( hashval +56, state->h64[7]);
  }
  
  return blake_SUCCESS;
}

blake_HashReturn blake_Final( blake_hashState * state, blake_BitSequence * hashval ) {
...
blake_HashReturn blake_Hash( int hashbitlen, const blake_BitSequence * data, blake_DataLength databitlen, 
		 blake_BitSequence * hashval ) {

  blake_HashReturn ret;
  blake_hashState state;

  ret = blake_Init( &state, hashbitlen );
  if ( ret != blake_SUCCESS ) return ret;

  ret = blake_Update( &state, data, databitlen );
  if ( ret != blake_SUCCESS ) return ret;

  ret = blake_Final( &state, hashval );
...
int main() {
  ...
  int i;
  blake_BitSequence data[144]; 
  blake_BitSequence hash[64];
