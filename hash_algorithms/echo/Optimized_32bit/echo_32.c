...
/********************************************************************************** 
 * Source file for the 32 bits optimized ansi C implementation of the ECHO hash 
 * function proposal.
 * Author(s) : Ryad Benadjila - Orange Labs - October 2008.
 **********************************************************************************/
...
echo_HashReturn SetSalt(echo_hashState* state, const echo_BitSequence* SALT){
...
echo_HashReturn echo_Init(echo_hashState *state, unsigned int hashbitlen)
{
	unsigned int i; /*,j*/
	//unsigned char endianess;
...
echo_HashReturn Compress(echo_hashState *state)
{ 	
...
echo_HashReturn echo_Update(echo_hashState *state, const echo_BitSequence *data, echo_DataLength databitlen){
...
echo_HashReturn echo_Final(echo_hashState *state, echo_BitSequence *hashval)
{
    	unsigned int i;
...
echo_HashReturn echo_Hash(int hashbitlen, const echo_BitSequence *data, echo_DataLength databitlen, echo_BitSequence *hashval) 
{ 
  echo_HashReturn S; 
  echo_hashState state; 
  S = echo_Init(&state, hashbitlen); 
  if(S != echo_SUCCESS) return S; 
  S = echo_Update(&state, data, databitlen); 
  if(S != echo_SUCCESS) return S; 
  return echo_Final(&state, hashval); 
}
