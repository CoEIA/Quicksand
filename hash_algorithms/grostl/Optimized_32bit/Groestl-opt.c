...
void Transform(grostl_hashState *ctx, 
	       const u8 *input, 
	       int msglen) {
...
void OutputTransformation(grostl_hashState *ctx) {
  int j;
  u32 *temp, *y, *z;
...
/* initialise context */
grostl_HashReturn grostl_Init(grostl_hashState* ctx,
		int hashbitlen) {
...
grostl_HashReturn grostl_Update(grostl_hashState* ctx,
		  const grostl_BitSequence* input,
		  grostl_DataLength databitlen) {
  int index = 0;
...
grostl_HashReturn grostl_Final(grostl_hashState* ctx,
		 grostl_BitSequence* output) {
  int i, j = 0, hashbytelen = ctx->hashbitlen/8;
  u8 *s = (grostl_BitSequence*)ctx->chaining;
...
/* hash bit sequence */
grostl_HashReturn grostl_Hash(int hashbitlen,
		const grostl_BitSequence* data, 
		grostl_DataLength databitlen,
		grostl_BitSequence* hashval) {
  grostl_HashReturn ret;
  grostl_hashState context;

  /* initialise */
  if ((ret = grostl_Init(&context, hashbitlen)) != Groestl_SUCCESS)
    return ret;

  /* process message */
  if ((ret = grostl_Update(&context, data, databitlen)) != Groestl_SUCCESS)
    return ret;
...
void PrintHash(const grostl_BitSequence* hash,
	       int hashbitlen) {
  int i;
