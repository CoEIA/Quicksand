...
double benchmark_Skein_256(uint8_t *input, uint64_t input_len, int number_of_tests){
    ...
    int test_hash;
    size_t hash_len_in_bits = 256;
    Skein_256_Ctxt_t* ctx;

    for(int i =0; i< number_of_tests ; i++){

        test_hash = skein_Hash(hash_len_in_bits ,input ,input_len , output);
...
int main(){
    ...
    for(int i=8; i< 9; i++){
        ...
        skein_time = benchmark_Skein_256(input, input_len[i], number_of_tests);
        printf("skein_256  time: "GRN"%f"reset"\n", skein_time);
